...
stClient UpdateClientData (stClient ClientInfo, string AccountNumber) {
...
void UpdateClientFromFile (vector <stClient> vClients, string AccountNumber, stClient Client, string Delim) {
    ...
    {
        if (vClient.AccountNumber == AccountNumber) 
            MyFile << ConvertRecordToLineForClient(UpdateClientData(vClient, AccountNumber), Delim) << endl;
        else 
...
int main() {
    ...
    {
        UpdateClientFromFile(vClients, AccountNumber, Client, "#//#");
    }
    else {
